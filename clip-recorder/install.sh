#!/bin/bash
# clip-recorder/install.sh
# Updated for Raspberry Pi OS Trixie (Debian 13) / kernel 6.x
# Changes from upstream:
#   - /boot/config.txt -> /boot/firmware/config.txt
#   - python3-rpi.gpio -> python3-lgpio + python3-gpiozero
#   - sudo pip install -> pip install --break-system-packages
...
# PEP 668: use --break-system-packages for pip installs outside a venv on Trixie
pip3 install --break-system-packages fonts font-roboto st7789 sounddevice

remove_from_config_text "dtoverlay=hifiberry-dac" "$CONFIG_TXT"

inform "Adding dtoverlay=adau7002-simple to $CONFIG_TXT"
add_to_config_text "dtoverlay=adau7002-simple" "$CONFIG_TXT"
...
success "Done! You may need to reboot for the overlay to take effect."
